...
import org.traccar.broker.IRabbitManager;
...
import org.traccar.forward.EventForwarderRabbitMQ;
...
import org.traccar.forward.PositionForwarderRabbitMQ;
    ...
    public static EventForwarder provideEventForwarder(Config config, Client client, ObjectMapper objectMapper) {
        if (config.hasKey(Keys.EVENT_FORWARD_URL)) {
            ...
            switch (forwardType) {
                ...
                case "rabbitMq":
                default:
                    return new EventForwarderRabbitMQ(config, objectMapper);
            }
        }
        return null;
    }
    ...
    public static PositionForwarder providePositionForwarder(Config config, Client client, ObjectMapper objectMapper) {
        if (config.hasKey(Keys.FORWARD_URL)) {
            switch (config.getString(Keys.FORWARD_TYPE)) {
                ...
                case "rabbitMq":
                default:
                    return new PositionForwarderRabbitMQ(config, objectMapper);
            }
        }
        return null;
    }
